...
OAuthOptions::OAuthOptions(QSharedPointer<Config> c)
{
    ...
    cbOpt1->setToolTip("Enable this option to prefer OAuth2 authentication. Ask your IT provider for these values.\n"\
                       "All fields except operator address is mandatory.");

    mainLayout->addRow(new QLabel("OAuth2 authentication URL"), leOpt1);
    leOpt1->setToolTip("Set the URL to use for OAuth auth requests. Mandatory");
    ...
    leOpt3->setToolTip("Application/client ID to use for authentication. Mandatory");

    mainLayout->addRow(new QLabel("OAuth2 scope"), leOpt4);
    leOpt4->setToolTip("Set the scope which this authentication should cover. Mandatory");

    mainLayout->addRow(new QLabel("File upload address"), leOpt5);
    leOpt5->setToolTip("The address to use for uploading SDEF measurement files. Mandatory");
